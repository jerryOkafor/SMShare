...
fun createUserDataStore(context: Context, json: Json): UserDataStore = UserDataStore(json) {
    context.filesDir.resolve(userDataStoreFileName).absolutePath
}

fun createUserSettingsStore(context: Context, json: Json): UserSettingsStore =
    UserSettingsStore(json) {
